      ...
      <UCard class="w-full">
        ...
        <div class="space-y-4">
          ...
          >
            ...
            <div v-else-if="problem.type == 'graph'">
              <div class="mb-4">
                <div>
                  <Desmos_comp
                    :functions="problem.desmos_expressions"
                  />
                </div>
              </div>
              <h3
              ...
              <p class="text-xs text-muted">
                Graph: {{ problem.desmos_expressions ?? 'unknown' }}
              </p>
              <div class="flex gap-2">
                <UButton
                  color="gray"
...
import Desmos_comp from '~/components/desmos_comp.vue'
  ...
  try {
    ...
    if (Array.isArray(rows)) {
      problems.value = rows.map((r) => {
        ...
        let desmos_expressions = []
        if (r.desmos_expressions) {
          try {
            desmos_expressions = JSON.parse(r.desmos_expressions)
            console.log(desmos_expressions)
          } catch (e) {
            console.warn('Failed to parse desmos_expressions:', r.desmos_expressions, e)
            desmos_expressions = []
          }
        }
        ...
        return {
          ...
          type: r.type ?? null,
          desmos_expressions: desmos_expressions // Now it's a proper array
        }
      })
    } else {
      problems.value = []
